...
exports.initVideoPlay = function(ctx) {
	...
	.fail(function(){console.log("Could not process video url");});
...
function startVideoSpeechStream() {
	...
	if (result.final) {
	    ...
	    console.log({text: result.alternatives[0].transcript, timestamp: result.alternatives[0].timestamps[0][1] + videoProps.currentTime});
	    videoProps.transcript.push({text: result.alternatives[0].transcript, timestamp: result.alternatives[0].timestamps[0][1] + videoProps.currentTime})
